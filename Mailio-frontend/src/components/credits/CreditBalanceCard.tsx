...
export function CreditBalanceCard({ footnote, className }: CreditBalanceCardProps) {
  ...
  const balance = showEnterprise ? user.enterprise!.creditBalance : user.creditBalance;
  const used = showEnterprise ? user.enterprise!.creditsUsed : user.creditsUsed;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {subline ? <p className="mt-0.5 text-xs text-gray-500">{subline}</p> : null}
        </div>
        {showEnterprise ? (
      ...
      <div className="mt-3 grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="mt-1 text-2xl font-semibold text-gray-900">{formatNumber(balance)}</p>
        </div>
        <div>
          <p className="text-xs text-gray-500">Used</p>
          <p className="mt-1 text-2xl font-semibold text-gray-900">{formatNumber(used)}</p>
        </div>
      </div>
