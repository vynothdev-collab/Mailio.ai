      ...
      <CardContent className="pt-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Verification activity across the enterprise
          </p>
        </div>
        {loading ? (
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3 mt-1">
        ...
        <span
          className={cn("text-sm font-bold", plan.isPopular ? "text-primary" : "text-foreground")}
        >
          {plan.name}
        </span>
      </div>
          ...
          {validityPlans.length > 0 && (
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Fixed credits with a validity period. Stacks with your current plan.
                </p>
              </div>
          ...
          {topupPlans.length > 0 && (
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Add extra credits to your existing plan's balance and expiry.
                </p>
              </div>
    ...
    <Card className="border-border/60">
      <CardContent className="pt-5 space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            All credit transactions for your enterprise
          </p>
        </div>
                ...
                entries.map((entry, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={cn(
                          "px-3 py-2.5 text-xs font-semibold whitespace-nowrap tabular-nums",
...
function ReallocationModal({
  ...
  const handleConfirm = async () => {
    ...
    const allocations = users.map((u) => ({
      userId: u.id,
      amount: parseInt(amounts[u.id] ?? "0", 10) || 0,
    }));
    ...
    for (const alloc of allocations) {
      ...
      if (u && alloc.amount < u.used) {
        setErr(
          `${u.name}'s allocation cannot be less than their used credits (${u.used.toLocaleString()}).`
        );
        return;
      }
    }
    if (total > result.creditBalance) {
      setErr(
        `Total allocations (${total.toLocaleString()}) exceed plan credits (${result.creditBalance.toLocaleString()}).`
      );
      return;
    }
        ...
        <div className="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              New plan has {result.creditBalance.toLocaleString()} credits. Adjust allocations for
              each member.
            </p>
          </div>
        </div>
              ...
              <div key={u.id} className="flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {u.email} · used {u.used.toLocaleString()}
                  </p>
                </div>
...
export function EnterpriseBillingView() {
  ...
  const [realloc, setRealloc] = useState<{ result: PurchasePlanResult; plan: BillingPlan } | null>(
    null
  );
            ...
            <CardContent className="pt-5 space-y-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Active plan + queued upcoming plans
                  </p>
                </div>
              </div>
                ...
                <div className="rounded-xl border border-primary/30 bg-primary/[0.03] p-3">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums">
                      ...
                      {subscription.activeBase.totalCredits.toLocaleString()} remaining
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {new Date(subscription.activeBase.startDate).toLocaleDateString(undefined, {
                      month: "short",
                      day: "numeric",
                      year: "numeric",
                    })}
                    {subscription.activeBase.endDate &&
                      ` → ${new Date(subscription.activeBase.endDate).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}`}
                  </p>
                </div>
              )}
              {subscription.activeTopups.length > 0 && (
                <div className="text-xs text-muted-foreground">
                  ...
                  {subscription.activeTopups.length > 1 ? "s" : ""}, total{" "}
                  ...
                  {subscription.queued.map((q) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground">
                        Starts{" "}
                        {new Date(q.startDate).toLocaleDateString(undefined, {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                        })}{" "}
                        • {q.totalCredits.toLocaleString()} credits
                      </span>
                    </div>
                  ))}
                </div>
