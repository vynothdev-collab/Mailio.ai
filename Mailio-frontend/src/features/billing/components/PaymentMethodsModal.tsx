...
import { CreditCard, Trash2, X, CheckCircle2, ChevronRight } from "lucide-react";
...
export function PaymentMethodsModal({ open, onClose }: Props) {
  ...
  function methodSub(m: SavedPaymentMethod) {
    if (m.type === "card") return `Expires ${String(m.expMonth).padStart(2, "0")} / ${m.expYear}`;
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md! w-full" showCloseButton>
        <DialogHeader>
          ...
          <DialogDescription>Manage your saved payment methods and add new ones.</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <div className="divide-y divide-border rounded-xl border border-border overflow-hidden">
              {methods.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center gap-3 px-3 py-3 bg-card hover:bg-muted/20 transition-colors"
                >
