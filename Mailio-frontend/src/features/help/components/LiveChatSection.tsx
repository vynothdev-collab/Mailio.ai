...
const SESSIONS: Session[] = [
  {
    ...
    messages: [
      {
        from: "agent",
        text: "Hi there! 👋 Welcome to emailanswers.ai support.\nHow can I help you today?",
        time: "10:01 AM",
      },
      {
        from: "user",
        text: "Hi, I have a question about bulk verification limits.",
        time: "10:02 AM",
      },
      {
        from: "agent",
        text: "Sure! On the PRO plan you get 10,000 verifications/month.\nWould you like more details?",
        time: "10:02 AM",
      },
      { from: "user", text: "Yes please. Can I upgrade anytime?", time: "10:03 AM" },
      {
        from: "agent",
        text: "Yes, you can upgrade anytime from the Billing settings.\nThe new limits will apply immediately.",
        time: "10:03 AM",
      },
    ],
  },
  {
    ...
    messages: [
      {
        from: "agent",
        text: "Hi there! 👋 Welcome to emailanswers.ai support. How can I help you today?",
        time: "2:10 PM",
      },
      { from: "user", text: "Hi, I didn't receive my invoice for April billing.", time: "2:11 PM" },
      {
        from: "agent",
        text: "I've resent the April invoice to your email. You should receive it within a few minutes.",
        time: "2:13 PM",
      },
    ],
  },
  {
    ...
    messages: [
      ...
      {
        from: "user",
        text: "My bulk verification job has been stuck at 0% for over 20 minutes.",
        time: "11:01 AM",
      },
      {
        from: "agent",
        text: "The job has completed successfully. Your results are now in the Results tab.",
        time: "11:06 AM",
      },
    ],
  },
  {
    ...
    messages: [
      ...
      {
        from: "agent",
        text: "Try logging in via an incognito window — that should clear the issue.",
        time: "9:32 AM",
      },
  ...
  },
  {
    ...
    messages: [
      ...
      {
        from: "agent",
        text: "Go to Results, find your job, and click the Download button. The CSV includes all status columns.",
        time: "3:02 PM",
      },
    ],
  },
...
export function LiveChatSection() {
  ...
  return (
    <div className="flex flex-col gap-3 xl:flex-row xl:items-start">
            ...
            return (
              ...
              >
                <div className="relative mt-0.5 shrink-0">
                  <div
                    className={`flex h-7 w-7 items-center justify-center rounded-full ${isActive ? "bg-[#0B47CF]" : "bg-[#EEF3FB]"}`}
                  >
                    <MessageCircle
                      size={12}
                      className={isActive ? "text-white" : "text-[#0B47CF]"}
                    />
                  </div>
                  {!s.resolved && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <p
                      className={`text-xs font-semibold truncate ${isActive ? "text-[#0B47CF]" : "text-[#111827]"}`}
                    >
                      {s.title}
                    </p>
                    {s.resolved && (
                      <span className="shrink-0 rounded-full bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">
                        Done
                      </span>
                    )}
                  </div>
      ...
      {}
      <div
        className="flex-1 rounded-2xl border border-[#DCE6F3] bg-white shadow-sm overflow-hidden flex flex-col"
        style={{ minHeight: 520 }}
      >
        {}
        <div className="flex items-center gap-3 border-b border-[#DCE6F3] px-4 sm:px-5 py-3 bg-white">
          <div className="relative shrink-0">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#0B47CF] text-white text-xs font-bold">
              SA
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-4 space-y-4 bg-[#F4F8FF]/30">
          {messages.map((m, i) => (
            ...
            >
              {m.from === "agent" && (
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#0B47CF] text-white text-[10px] font-bold mt-0.5">
                  SA
                </div>
              )}
              <div className="max-w-[75%] sm:max-w-[65%] space-y-1">
                <div
                  className={`rounded-2xl px-3.5 py-2.5 text-xs sm:text-sm leading-relaxed whitespace-pre-line
                  ${
                    m.from === "user"
                ...
                <p
                  className={`text-[10px] text-muted-foreground flex items-center gap-1 ${m.from === "user" ? "justify-end" : ""}`}
                >
                  {m.time}
      ...
      <div className="xl:w-72 flex flex-col gap-3">
        ...
        <div className="rounded-2xl border border-[#DCE6F3] bg-white shadow-sm p-4 sm:p-5">
          <div className="flex items-start gap-3 mb-4">
            <div className="relative shrink-0">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#0B47CF] text-white font-bold">
                SA
              </div>
          ...
          <p className="text-xs text-muted-foreground leading-relaxed mb-3">
            Our team is available Mon–Fri, 9:00 AM – 6:00 PM (UTC)
          </p>
        ...
        <div className="rounded-2xl border border-[#DCE6F3] bg-white shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-[#DCE6F3]/60">
            {QUICK_ACTIONS.map(({ icon: Icon, label, sub }) => (
              <button
                key={label}
                className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-[#F4F8FF] transition-colors group"
              >
