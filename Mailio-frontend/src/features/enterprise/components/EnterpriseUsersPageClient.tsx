...
function EditUserModal({
  ...
  user: EnterpriseUser;
  onClose: () => void;
  onSaved: (updated: EnterpriseUser) => void;
}) {
...
function ChangePasswordModal({ user, onClose }: { user: EnterpriseUser; onClose: () => void }) {
      ...
      <DialogContent className="max-w-sm!" showCloseButton>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Field label="New Password">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Share this securely with the user after updating.
            </p>
          </Field>
        ...
        {creditSummary && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatTile
              ...
              value={creditSummary.totalPurchased.toLocaleString()}
              icon={TrendingUp}
              iconBg="bg-blue-50"
              iconColor="text-blue-600"
              loading={false}
            ...
            <StatTile
              ...
              value={creditSummary.totalAllocated.toLocaleString()}
              icon={PieChart}
              iconBg="bg-purple-50"
            ...
            <StatTile
              label="Used by Users"
              value={creditSummary.totalUsed.toLocaleString()}
              icon={BarChart2}
              iconBg="bg-orange-50"
              iconColor="text-orange-500"
              loading={false}
            ...
            <StatTile
              label="Admin Usable"
              value={creditSummary.adminUsable.toLocaleString()}
              icon={ShieldCheck}
              iconBg="bg-emerald-50"
              iconColor="text-emerald-600"
              loading={false}
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {loading
                    ? "Loading…"
                    : `${users.length} member${users.length !== 1 ? "s" : ""} in your enterprise`}
                </p>
              </div>
              ...
              {loading ? (
                <div className="space-y-0">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div
                      key={i}
                      className="flex gap-4 border-b border-border/50 last:border-0 px-3 py-3"
                    >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">
                            {u.creditExpiresAt
                              ? new Date(u.creditExpiresAt).toLocaleDateString()
                              : "—"}
                          </td>
                          <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">
        ...
        body={
          activeUser && (
            <p>
              ...
              <span className="text-xs">({activeUser.email})</span>? Their account will be
          ...
          activeUser && (
            <p>
              {activeUser.isActive ? (
                <>
                  Deactivating{" "}
                  <span className="font-semibold text-foreground">{activeUser.name}</span> will
                  prevent them from signing in. Their data and credit allocation will be preserved.
                ...
                <>
                  Activating{" "}
                  <span className="font-semibold text-foreground">{activeUser.name}</span> will
                  restore their ability to sign in and use their allocated credits.
                </>
              )}
            </p>
          )
