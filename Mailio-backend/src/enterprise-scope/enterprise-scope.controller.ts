...
export class EnterpriseScopeController {
  ...
  @ApiOperation({
    summary: 'Create an ENTERPRISE_USER under the calling admin enterprise.',
  })
  createUser(@CurrentUser() admin: User, @Body() dto: CreateEnterpriseUserDto) {
  ...
  @ApiOperation({
    summary: 'Add an existing platform user to the calling admin enterprise.',
  })
  addExistingUser(@CurrentUser() admin: User, @Body() body: { email: string }) {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Soft-delete a user (blocks login, keeps in enterprise).',
  })
  softDeleteUser(@CurrentUser() admin: User, @Param('userId') userId: string) {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Toggle active/inactive status for an enterprise user.',
  })
  toggleUserStatus(
  ...
  @Get('users')
  @ApiOperation({
    summary: 'List all users belonging to the calling admin enterprise.',
  })
  listUsers(
    @CurrentUser() admin: User,
  ...
  @Get('overview')
  @ApiOperation({
    summary: 'Aggregate dashboard stats for the calling admin enterprise.',
  })
  ...
  @Get('credits/ledger')
  @ApiOperation({
    summary: 'Read-only credit transactions for the enterprise.',
  })
  getLedger(
    @CurrentUser() admin: User,
    @Query('page', new ParseIntPipe({ optional: true })) page = 1,
  ...
  @Get('files')
  @ApiOperation({
    summary: 'List all bulk-verify jobs across the enterprise (all members).',
  })
  listFiles(
    @CurrentUser() admin: User,
    @Query('page', new ParseIntPipe({ optional: true })) page = 1,
