...
export class AddAdminTables1779800978531 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX "public"."idx_email_lists_user_active"`,
    );
    await queryRunner.query(`DROP INDEX "public"."idx_emails_user_active"`);
    await queryRunner.query(
      `DROP INDEX "public"."idx_emails_address_completed"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."admins_role_enum" AS ENUM('SUPER_ADMIN', 'ADMIN')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX "public"."uq_users_provider_provider_id"`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."auth_provider" RENAME TO "auth_provider_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."users_provider_enum" AS ENUM('LOCAL', 'GOOGLE', 'LINKEDIN')`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "provider" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "provider" TYPE "public"."users_provider_enum" USING "provider"::"text"::"public"."users_provider_enum"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX "public"."idx_email_otps_email_purpose"`,
    );
    await queryRunner.query(
      `ALTER TYPE "public"."otp_purpose" RENAME TO "otp_purpose_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."email_otps_purpose_enum" AS ENUM('SIGNUP_VERIFY', 'PASSWORD_RESET')`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" TYPE "public"."email_otps_purpose_enum" USING "purpose"::"text"::"public"."email_otps_purpose_enum"`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" SET DEFAULT 'SIGNUP_VERIFY'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."otp_purpose_old" AS ENUM('SIGNUP_VERIFY', 'PASSWORD_RESET')`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" TYPE "public"."otp_purpose_old" USING "purpose"::"text"::"public"."otp_purpose_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "email_otps" ALTER COLUMN "purpose" SET DEFAULT 'SIGNUP_VERIFY'`,
    );
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."otp_purpose_old" RENAME TO "otp_purpose"`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_email_otps_email_purpose" ON "email_otps" ("email", "purpose") `,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."auth_provider_old" AS ENUM('LOCAL', 'GOOGLE', 'LINKEDIN')`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "provider" DROP DEFAULT`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "provider" TYPE "public"."auth_provider_old" USING "provider"::"text"::"public"."auth_provider_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ALTER COLUMN "provider" SET DEFAULT 'LOCAL'`,
    );
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."auth_provider_old" RENAME TO "auth_provider"`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "uq_users_provider_provider_id" ON "users" ("provider", "provider_id") WHERE (provider_id IS NOT NULL)`,
    );
    await queryRunner.query(
      `ALTER TABLE "users" ADD "reset_password_expires_at" TIMESTAMP WITH TIME ZONE`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_emails_address_completed" ON "emails" ("address", "processed_at") WHERE ((status = 'COMPLETED'::emails_status_enum) AND (is_deleted = false))`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_emails_user_active" ON "emails" ("user_id") WHERE (is_deleted = false)`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_email_lists_user_active" ON "email_lists" ("user_id") WHERE (is_deleted = false)`,
    );
  }
}
